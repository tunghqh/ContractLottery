...
contract Lottery {
    ...
    event newTicket(uint id ,uint [3] number,uint roundID , address player, uint price ) ;
    event newRound(uint id,uint [3] ramdom , uint total);
    ...
    function buyTicket(uint[3] memory numbers) public payable{
        ...
        Ticket memory ticket = Ticket(TicketID, numbers, RoundId,msg.sender,TicketPrice,'none',0);
        
        tickets.push(ticket);
    
        emit newTicket(TicketID,numbers,RoundId,msg.sender,TicketPrice) ;
        
        TotalBNB += TicketPrice ;
        TotalTicket += 1;
    ...
    function dial() public  {
        ...
        uint hoahongadmin = TotalBNB * 2 / 10 ;
        
        payable(owner).transfer(hoahongadmin) ;
            
        TotalBNB = TotalBNB - hoahongadmin ;
        ...
        Round memory round  = Round(RoundId,numbers,TotalBNB) ;
    ...
    function _pickWinner() private{
        ...
        for(uint m = 0 ; m < tickets.length; m++){
            ...
            if(winner.id == tickets[m].roundID){
                uint jackpot = TotalBNB / countJackpots  ;
                if(keccak256(abi.encodePacked(tickets[m].win)) == keccak256(abi.encodePacked("Jackpots"))){
                    tickets[m].prize = jackpot ;
                    payable(tickets[m].player).transfer(jackpot);
                    TotalBNB = TotalBNB - jackpot ;
